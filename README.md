...
- **Bộ nhớ & Dọn dẹp** (Dev & API): xem dung lượng theo loại/khách hàng, dọn file rác, **khôi phục media** từ gói (kể cả gói trên Google Drive).
- Tự động: bật service backup (lệnh trên) hoặc `update.sh` luôn backup trước mỗi lần cập nhật.

## Chuyển sang VPS mới / Khôi phục toàn bộ (Disaster Recovery)
Gói backup (`database.sql`) chứa **toàn bộ**: tin nhắn, danh bạ, người dùng, phân quyền, **cấu hình**, tích hợp, tài khoản Zalo, tag. Để VPS mới chạy y như cũ:
1. Cài app trên VPS mới (1 lệnh như trên).
2. **Giữ nguyên 3 secret** trong `.env` — copy từ `.env` cũ: `ENCRYPTION_KEY`, `JWT_SECRET`, `FB_TOKEN_ENC_KEY`. *(Khoá AI/Facebook/Drive được mã hoá bằng `ENCRYPTION_KEY` — dùng key mới khác sẽ phải nhập lại các khoá đó.)*
3. **Giữ nguyên domain kho file** (`S3_DOMAIN`) để URL ảnh trong tin nhắn vẫn trỏ đúng. (Domain app `APP_DOMAIN` đổi tuỳ ý.)
4. Nạp DB: `docker compose -p uaemcrm exec -T db psql -U crmuser uaemcrm < database.sql`, rồi đẩy lại `media/` vào MinIO (giữ nguyên key).
5. Tài khoản Zalo: đăng nhập lại bằng QR (session Zalo thường đã hết hạn).
> Chi tiết kèm trong `README.txt` của mỗi gói backup.

## Bảo mật (khuyến nghị)
- **Lần đầu** mở web → tạo tài khoản **chủ tổ chức** (mật khẩu ≥8 ký tự). Sau đó endpoint thiết lập tự khoá.
- Đăng nhập có **chống brute-force** (giới hạn lần thử/IP). Mỗi tài khoản được **phân quyền (RBAC)** riêng.
- **Nhiều lớp (tuỳ chọn):** đặt **Cloudflare Zero Trust (Access)** trước `crm.<domain>` để bắt buộc định danh
  *trước khi* tới trang login → an toàn hơn. ⚠️ Khi đó phải **Bypass** các path máy chủ ngoài gọi:
  `/api/v1/integrations/facebook/webhook`, `/api/public/*`, `/health`, `/api/v1/status` (kẻo hỏng webhook/tích hợp).
- **`files.<domain>` KHÔNG đặt Access** — Zalo CDN cần tải ảnh ẩn danh; chỉ **proxy qua Cloudflare + WAF/rate-limit**.
  Key file là UUID ngẫu nhiên, không liệt kê được, không cho upload ẩn danh.
